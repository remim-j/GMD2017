...
private static final int NUM_CORES = Runtime.getRuntime().availableProcessors();
					...
					public void run() {
						if (idStitchFromSider != null) {
							for (String s : idStitchFromSider){
								...
								usefulMedicines.add(drugName,"Sider + ATC + Stitch");
							}
				    	}
					}
				});	    	
											...
											Future<?> future5 = executorService111.submit(
												new Runnable() {
													public void run() {
														...
														usefulMedicines.add(label,"Omim_onto + Sider + ATC + Stitch");
													}
												});
										}
									}
	 									...
	 									for (final String s : sideEffectStitchID) {
	 										Future<?> future7 = executorService.submit(
	 											new Runnable() {
	 												public void run() {
	 													String label=ReadStitch.getATCNameByStitchID(s);
	 													possibleOriginSideEffect.add(label,"Omim_Onto + sider + ATC + Stitch");
	 												}
	 											});
	...
	/*********do  search with a fix symptom******/
	
public static void doSearchFIX(String userInput) {
		...
		try {
			...
			ArrayList<String> idMedicineSideEffect = AccesSider.idMedicineSideEffectFIX(userInput);
			...
			final ArrayList<String> idStitchFromSider = AccesSider.getStitchIDByConceptNameFIX(userInput);
			final ArrayList<String> idSymptomHpObo = ReadHpObo.getIdFIX("name",userInput);
			...
			ArrayList<String> cuiMedicineOmimOnto = ReadOmimOnto.SymptomToCUIFIX(userInput);
			List<Future<?>> listfutures3 = new LinkedList<Future<?>>();
			...
			/***********************************************************************************/
			/** we enter in Omim_onto with CUI**/
			
			//By the way we construct the list of diseaseIdOrpha
			ArrayList<String> diseaseIdOrpha = AccesOrphaDataBase.GetDeseaseIdByClinicalSignFIX(userInput);
			final List<Future<?>> listfutures4 = new LinkedList<Future<?>>();
			...
			ArrayList<String> diseaseIdOmim = ReadOmim.getNOFIX("CS",userInput);
			if (diseaseIdOmim != null) {
	            for (String s : diseaseIdOmim) {
	            	...
	            	if (!possibleDiseases.alreadySave(s)) {
	            		...
	            		if (diseaseLabelFromHpoAnnot!= null) {
	            			...
	            			ArrayList<String> diseaseFromOmim = ReadOmim.getTIFIX("NO",s);
	            			if (diseaseFromOmim != null) {
	            				for (String s1:diseaseFromOmim) {
	            					possibleDiseases.add(s1, "Omim");
	            				}
